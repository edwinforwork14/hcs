...
-- Run this section (and the one above) in the SQL Editor.
-- ============================================================

-- Private storage bucket for chat attachments.
-- Access is granted per-file via signed URLs generated by the server
-- (service role), so no public policies are required.
insert into storage.buckets (id, name, public)
values ('chat-attachments', 'chat-attachments', false)
on conflict (id) do nothing;

-- Attachment metadata is stored inside the message `content` column as JSON
-- ({"attachment": {...}, "text": "..."}), so no schema changes are required.
